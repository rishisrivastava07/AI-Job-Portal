...
import com.rishiproject.job.Services.JobService;
...
public class JobController {
    ...
    @GetMapping
    public ResponseEntity<List<JobResponse>> getJobs(@ModelAttribute JobSearchRequest jobSearchRequest) {
        return ResponseEntity.status(HttpStatus.OK)
                .body(jobService.getJobs(jobSearchRequest));
    }

    @GetMapping("company/{companyId}")
    public ResponseEntity<List<JobResponse>> getJobsByCompany(@PathVariable Long companyId) {
        return ResponseEntity.status(HttpStatus.OK)
                .body(jobService.getJobsByCompany(companyId));
    }

    @GetMapping("admin")
    public ResponseEntity<List<JobResponse>> getAllJobs() {
        return ResponseEntity.status(HttpStatus.OK)
                .body(jobService.getAllJobsAdmin());
    }
    ...
    public ResponseEntity<ApiResponse> deleteJob(
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(new ApiResponse("Job Deleted successfully", true));
    }
}
